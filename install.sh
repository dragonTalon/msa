...
get_latest_version() {
    ...
    response=$(curl -sSL --max-time 30 https://api.github.com/repos/dragonTalon/msa/releases 2>/dev/null) || true
...
install_msa() {
    ...
    # 如果无法获取版本，直接报错并提供手动下载链接
    if [ -z "$version" ]; then
        echo ""
        error "无法获取版本信息"
        echo ""
        echo "可能原因：网络问题或 GitHub 访问限制"
        echo "建议方案：请检查网络连接后重试"
        echo ""
        echo "手动下载链接：https://github.com/dragonTalon/msa/releases"
        exit 1
    fi
    ...
    info "正在下载..."
    if ! curl -fsSL --max-time 60 --progress-bar -o "$tmp_file" "$download_url"; then
        echo ""
        error "下载失败"
        echo ""
        echo "可能原因：网络问题或 GitHub 访问限制"
        echo "建议方案：请手动下载对应平台的二进制文件"
        echo ""
        echo "手动下载链接：https://github.com/dragonTalon/msa/releases"
        exit 1
    fi
