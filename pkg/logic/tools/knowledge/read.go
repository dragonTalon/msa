...
func ReadKnowledgeFile(ctx context.Context, param *ReadKnowledgeFileParam) (string, error) {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			return "", fmt.Errorf("%w: %s", ErrFileNotFound, param.Path)
		}
		message.BroadcastToolEnd("read_knowledge_file", "", err)
	...
	if info.IsDir() {
		...
		return "", fmt.Errorf("路径是目录，不是文件: %s", param.Path)
	}
	...
	// 构建返回结果
	result := formatReadResult(param.Path, metadata, body, info)

	message.BroadcastToolEnd("read_knowledge_file", result, nil)
	return result, nil
}

// formatReadResult 格式化读取结果
