...
type Transaction struct {
	...
	Price      int64             `gorm:"type:INTEGER;not null" db:"price"`       // 交易价格（分）
	Amount     int64             `gorm:"type:INTEGER;not null" db:"amount"`      // 交易金额（分）= quantity × price
	Fee        int64             `gorm:"type:INTEGER;not null;default:0" db:"fee"`         // 手续费（分）
