...
系统 MUST 能够扫描指定目录下的所有 Skill 文件并解析其元数据。
...
系统 MUST 正确解析 SKILL.md 文件的 YAML frontmatter。
...
Skill 内容（SKILL.md body）MUST 按需加载，不在启动时全部加载到内存。
...
系统 MUST 维护一个 Skill 注册表，支持按名称查询和遍历。
...
系统 MUST 区分内置 Skills 和用户自定义 Skills。
