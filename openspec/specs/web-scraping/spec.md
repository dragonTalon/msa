...
系统 MUST 提供 Agent 抓取指定网页内容的能力，以获取详细信息，SHALL 使用真实浏览器引擎渲染页面并提取清洁的正文内容。

## Requirements

### Requirement: 抓取网页内容

系统 MUST 提供 Agent 抓取指定网页内容的能力，以获取详细信息。
...
#### Scenario: 处理 JavaScript 渲染页面

- **当** 目标网页使用 JavaScript 动态渲染内容
- **那么** 系统必须使用浏览器渲染等待页面完全加载
- **并且** 提取渲染后的文本内容
- **并且** 不应返回未渲染的 HTML 源码

#### Scenario: 无效或不可访问的 URL
...
系统 MUST 从网页中提取主要内容，去除导航、广告、脚本等无关信息。
...
系统 MUST 控制返回内容的长度，避免超出 LLM 的 token 限制。
...
内容提取逻辑 MUST 与抓取逻辑解耦，便于针对不同网站优化。
...
系统 MUST 使用真实的浏览器引擎渲染页面，以支持现代网页技术。

#### Scenario: 使用浏览器渲染

- **当** 系统需要渲染网页
- **那么** 必须使用浏览器驱动（如 chromedp）
- **并且** 使用 headless 模式运行
- **并且** 支持拦截和禁用图片加载以提升性能
...
### Requirement: 安全性和资源限制

系统 MUST 限制资源使用，防止恶意或异常网页导致问题。
...
网页抓取工具 MUST 与搜索工具良好配合。

#### Scenario: 从搜索结果获取详情

- **当** Agent 先调用搜索工具获取搜索结果
- **并且** 对某个结果感兴趣，调用网页抓取工具
