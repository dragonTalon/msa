...
账户总览计算逻辑 SHALL 正确计入持仓市值，并在价格获取失败时提示用户。

## Requirements

### Requirement: 总资产计算必须包含持仓市值

总资产 MUST 正确计算为：可用余额 + 锁定金额 + 持仓市值。
...
### Requirement: GetAccountTotalValue 返回完整信息

`GetAccountTotalValue` MUST 返回 `AccountValueResult` 结构体，包含总资产、持仓市值、价格获取失败的股票列表。
...
### Requirement: 部分价格失败时仍计算可用市值

即使部分股票价格获取失败，系统 SHALL 计算能获取价格的股票市值，并明确提示哪些股票价格缺失。
...
### Requirement: AccountSummaryData 包含失败信息

`AccountSummaryData` MUST 包含 `FailedStocks` 和 `Warning` 字段，让用户知晓数据不完整。
...
### Requirement: 价格失败不中断工具执行

价格获取失败 SHALL 作为警告信息继续执行，不中断整个工具调用流程。

#### Scenario: 价格失败不中断流程

- **当** `fetchAllPrices` 返回错误或部分股票价格获取失败
- **那么** 继续执行，将失败信息记录在返回数据的 `Warning` 字段中
- **那么** 工具正常返回（不抛出 error，不触发 EventToolError）

---

### Requirement: 向后兼容

API 响应新增字段 SHALL 为可选字段，不影响现有调用方。
